...
$(document).ready(function() {
  ...
  $.ajax({url:url, dataType:"jsonp"}).then(function(data) {
    ...
    if(temp > 100)
      ...
      //extra credit therm color (temp is the current temperature from line 25)
    var thermColor = (temp > 85 ) ? ('red') : (temp < 65) ? ('blue') : ('grey');

    document.getElementById('thermometer_inner').style.height = temp_today;
    document.getElementById('thermometer_inner').style.color = thermColor;
    document.getElementById('precip_today').innerHTML = data.current.precip + '%';
    document.getElementById('humidity_today').innerHTML = data.current.wind_speed;
   ...
   var HTMLcard = "";
        for(var counter = 0; counter < 5; counter++){
          var forecastData = getKey(counter);
          HTMLcard += '<div style="width:20%;">' +
              '<div class="card">'+
                '<div class="card-body">' +
        ...
        document.getElementById("5_day_forecast").innerHTML = HTMLcard;
  })
});
